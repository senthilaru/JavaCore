...
public class ImageReader{
	...
	public void oldProcessImageThreads(){
		try {
			...
			System.out.println("bufferedImage.getHeight():"+bufferedImage.getHeight() +"::bufferedImage.getWidth():"+bufferedImage.getWidth());
			
			final WritableRaster raster = bufferedImage.getRaster();
			long startTs = System.currentTimeMillis();
			System.out.println("raster.getHeight():"+raster.getHeight() +"::raster.getWidth():"+raster.getWidth());
			
			int[] srcImgPixels = bufferedImage.getRGB(0, 0, bufferedImage.getWidth(), bufferedImage.getHeight(), null, 0, 0);
			
			OldSegmentProcess(bufferedImage, raster, 0, 0, raster.getWidth()/2, raster.getHeight()/2);
			OldSegmentProcess(bufferedImage, raster, raster.getWidth()/2, 0, raster.getWidth(), raster.getHeight()/2);
			OldSegmentProcess(bufferedImage, raster, 0, raster.getHeight()/2, raster.getWidth()/2, raster.getHeight());
			OldSegmentProcess(bufferedImage, raster, raster.getWidth()/2, raster.getHeight()/2, raster.getWidth(), raster.getHeight());
			
			
			ImageIO.write(bufferedImage, "jpg", new BufferedOutputStream(Files.newOutputStream(Paths.get(processedImageFile.toURI()),StandardOpenOption.CREATE) ));
	...
	public void processImageConcurrently(){
		try {
			...
			bufferedImage.getRGB(0, 0, bufferedImage.getWidth(), bufferedImage.getHeight(), srcImgPixels, 0, bufferedImage.getWidth());
			...
			BufferedImage dstImage = bufferedImage.getSubimage(0, 0, bufferedImage.getWidth(), bufferedImage.getHeight());
			...
			ImageIO.write(dstImage, "jpg", new BufferedOutputStream(Files.newOutputStream(Paths.get(processedImageFile.toURI()),StandardOpenOption.CREATE) ));
...
//			blue = (int) (blue * 0.1);
	...
	public void OldSegmentProcess(BufferedImage bufferedImage, WritableRaster raster,int startX, int startY, int imgWidth, int imgHeight ) {
...
//					System.out.println("color:["+ color.getAlpha() +" - " +color.getRed() + "," + color.getGreen() + "," + color.getBlue()+"]");
				...
				int[] newRgb = new int[]{(int)(0),0,color.getBlue()};
